        ...
        NPointCrossover(final int n, final RandomGenerator rng) {
            this(new Function<Integer, Iterable<Integer>>() {
                ...
                public Iterable<Integer> apply(final Integer bitStringLength) {
                    return Samplings.randomWithoutReplacement(rng).sample(ContiguousSet.create(Range.closedOpen(0, bitStringLength), DiscreteDomain.integers()), n
                    );
                }
            });
        }
            ...
            this(new Function<Integer, Iterable<Integer>>() {
                ...
                public Iterable<Integer> apply(final Integer bitStringLength) {
                    if (bitStringLength * p < 0.01) {
                        return Samplings.randomWithoutReplacement(rng).sample(ContiguousSet.create(Range.closedOpen(0, bitStringLength), DiscreteDomain.integers()), new BinomialDistribution(rng, bitStringLength, p).sample()
                        );
                    } else {
