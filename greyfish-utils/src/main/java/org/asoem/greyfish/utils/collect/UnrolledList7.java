...
/**
 * This is a {@link FunctionalList FunctionalList} implementation for lists of size 6. The predictable size allows some
 * operations to unroll the list for performance optimizations.
 *
 * @param <E> the element type
 */
final class UnrolledList7<E> extends DelegatingImmutableFunctionalList<E> implements Serializable {

    private final List<E> delegate;

    UnrolledList7(final Iterable<? extends E> elements) {
        checkNotNull(elements);
        this.delegate = ImmutableList.copyOf(elements);
        checkArgument(delegate.size() == 7);
    }
    ...
    public Optional<E> findFirst(final Predicate<? super E> predicate) {
        ...
        final E e6 = delegate.get(6);
        if (predicate.apply(e6)) {
