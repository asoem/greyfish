...
public class AdaptiveDiversificationClassicalModel extends AbstractInjectedModelFactory {
    ...
    @Inject(optional = true)
    @ScenarioParameter("habitat_distance")
    private String habitat_distance = "1.5";

    @Inject(optional = true)
    @ScenarioParameter("habitat_sd")
    private String habitat_sd = "1.0";
    ...
    public Scenario get() {
        ...
        final BasicScenario.Builder scenarioBuilder = BasicScenario.builder(this.getClass().getSimpleName(), tiledSpace);
                ...
                .addActions(
                        Suicide.with()
                                ...
                                .executesIf(GenericCondition.evaluate(new Callback<GenericCondition, Boolean>() {
                                    private final double stdDev = Double.valueOf(habitat_sd);
                                    private final double distance = Double.valueOf(habitat_distance);
                                    ...
                                    private double gaussian(double value, double distance) {
                                        return FastMath.exp(-FastMath.pow(value + distance, 2) / 2 * FastMath.pow(stdDev, 2));
                                    }
                                }))
                                .build(),
