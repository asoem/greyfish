...
public class BasicScenario implements Scenario {
    ...
    public Iterable<Agent> getPlaceholder() {
        return Iterables.filter(space.getOccupants(), Agent.class);
    }

    @Override
    public Iterable<Agent> getPlaceholder(TileLocation location) {
        return Iterables.filter(space.getOccupants(location), Agent.class);
    }
            ...
            public boolean apply(final Agent placeholder) {
                return Iterables.any(locations, new Predicate<TileLocation>() {
                    ...
                    public boolean apply(TileLocation o) {
                        return o.covers(placeholder.getProjection());
                    }
                });
            }
        });
        ...
        public BasicScenario build() {
            checkState(Iterables.all(projections, new Predicate<AgentProjectionPair>() {
                ...
                public boolean apply(AgentProjectionPair object2D) {
                    return space.covers(object2D.projection);
                }
            }));
