...
public class FinalizedAgent extends AbstractAgent implements MovingObject2D, Preparable<Simulation> {

    private static final Logger LOGGER = LoggerFactory.getLogger(FinalizedAgent.class);
    ...
    private FinalizedAgent(FinalizedAgent agent, CloneMap map) {
        super(map.clone(agent, FinalizedAgent.class), map);

        Simulation simulation = checkNotNull(agent.simulation);
    ...
    public FinalizedAgent(Prototype prototype, Simulation simulation) {
        super();
        ...
        for(GFAction action : clone.getActions()) {
            super.addAction(action);
        }

        for(GFProperty property : clone.getProperties()) {
    ...
    public static FinalizedAgent newInstance(Prototype prototype, Simulation simulation) {
        return new FinalizedAgent(prototype, simulation);
    }
    ...
    public DeepCloneable deepCloneHelper(CloneMap map) {
        return new FinalizedAgent(this, map);
    }
    ...
    public Iterable<MovingObject2D> findNeighbours(double range) {
        return simulation.findObjects(this, range);
    }
}
