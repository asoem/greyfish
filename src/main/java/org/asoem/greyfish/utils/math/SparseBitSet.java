...
//==============================================================================
    ...
    // This number is fixed by the size of longs
    static {
        assert (1 << LEVEL4) == LENGTH4 : "LEVEL4 value is incorrect!";
    }
...
/*	Programming notes:
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof SparseBitSet)) return false;
    ...
    public void flip(int i) {
        ...
        long[][] a2;
        if ((a2 = bits[w1]) == null) a2 = bits[w1] = new long[LENGTH2][];
        long[] a3;
    ...
    public int nextClearBit(int i) {
        ...
        if (i < 0) throw new IndexOutOfBoundsException("i=" + i);
    ...
    public String statistics(String[] values) {
        ...
        for (int i = 0; i != result.length(); ++i)
            if (result.charAt(i) == '_') result.setCharAt(i, ' ');
...
//==============================================================================
    ...
    protected static void throwIndexOutOfBoundsException(int i, int j)
        ...
        String s = "";
        if (i < 0) s += "(i=" + i + ") < 0";
        if (i == Integer.MAX_VALUE) s += "(i=" + i + ")";
        if (j < 0) s += (s.length() == 0 ? "" : ", ") + "(j=" + j + ") < 0";
        if (i > j) s += (s.length() == 0 ? "" : ", ") + "(i=" + i + ") > (j=" + j + ")";
    ...
    protected final void setScanner(int i, int j, SparseBitSet b,
        ...
        while (i < j) {
                ...
                while (u2 != limit2) {
                    ...
                    if ((!haveA3 && !haveB3 && f_op_f_eq_f
                        ...
                        if (notFirstBlock && notLastBlock)
                            ...
                            else isZero = op.block(base3, 0, LENGTH3, a3, b3);
            ...
            if (i < 0) i = Integer.MAX_VALUE;        //	Don't go over the end
        } /* end while( i < j ) */
...
//==============================================================================
    ...
    private void writeObject(ObjectOutputStream s) throws IOException, InternalError {
        ...
        if (count != 0) throw new InternalError("count of entries not consistent");
...
//=============================================================================
    ...
    protected abstract class AbstractStrategy {
        ...
        protected final boolean isZeroBlock(long[] a3) {
            for (long word : a3) if (word != 0L) return false;
            return true;
        }
    }
    ...
    protected class ClearStrategy extends AbstractStrategy {
        ...
        protected boolean block(int base, int u3, int v3, long[] a3, long[] b3) {
            if (u3 != 0 || v3 != LENGTH3)         //	Optimisation
                for (int w3 = u3; w3 != v3; ++w3) a3[w3] = 0L;
            return true;
        }
    }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    protected class FlipStrategy extends AbstractStrategy {
        ...
        protected boolean block(int base, int u3, int v3, long[] a3, long[] b3) {
            boolean isZero = true;            //	Presumption
            for (int w3 = u3; w3 != v3; ++w3) isZero &= (a3[w3] ^= ~0L) == 0L;
            return isZero;
        }
    }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    protected class XorStrategy extends AbstractStrategy {
        ...
        protected boolean block(int base, int u3, int v3, long[] a3, long[] b3) {
            boolean isZero = true;            //	Presumption
            for (int w3 = u3; w3 != v3; ++w3) isZero &= (a3[w3] ^= b3[w3]) == 0;
            return isZero;
