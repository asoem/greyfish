...
public class ResourceInteractionSpec extends Specification<ContractNetInitiatorAction> {
    public class NormalInteraction {
        ...
        final Prototype consumer = Prototype.with().population(Population.newPopulation("TestPop1", Color.black)).addProperties(energyStorage).addActions(consumptionAction).build();
        ...
        final Prototype provider = Prototype.with().population(Population.newPopulation("TestPop2", Color.black)).addProperties(resourceProperty).addActions(provisionAction).build();
